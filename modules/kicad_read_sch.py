# This script reads a KiCAD schematic file, extracts the symbols, wires, and labels, and generates a Python script with the proposed code representation that can be used to reconstruct the schematic using a custom interface. 
# The script also handles the connections between symbols and labels based on the wires in the schematic. It uses a breadth-first search to find all connected components in the schematic and processes each component separately. 
# The output Python script includes code to place symbols, add wires, and connect labels to pins based on their relative positions. The script can be customized to use definite locations for symbols and wires if needed.
...
class code_generator:
    ...
    def __init__(self, module_name, sch_ref_path, output_path, kicad_sch_filename=None):
        ...
        self.kicad_sch_filename = kicad_sch_filename # KiCAD schematic filename
    ...
    def work(self):
        ...
        # lines.append('### Set the schematic filename ###')
        # if self.kicad_sch_filename:
        #     lines.append(f'set_schematic_filename("{self.kicad_sch_filename}")')
        # else:
        #     lines.append('set_schematic_filename()')
        ...
        for center_symbol, symbol_set, label_set, wire_set in subgraphs:
            ...
            if not definite_location and not definite_wires:   
...
def debug():
    ...
    out_sch = sch_path.parent / f"{stem}_out.kicad_sch"

    generator = code_generator(
        module_name=module_name,
        sch_ref_path=str(sch_path),
        output_path=str(out_py),
        kicad_sch_filename=str(out_sch),
    )
    print(
...
if __name__ == "__main__":
    ...
    if DEBUG:
        ...
        proj_path = Path(os.environ["PROJECT_PATH"])
        base_dir = proj_path / "dataset_new"
        ...
        for module_dir in sorted(p for p in base_dir.iterdir() if p.is_dir()):
            ...
            for sch_path in sorted(module_dir.glob("sch_*_*.kicad_sch")):
                ...
                stem = sch_path.stem  
                if not definite_wires and not definite_location:
                    out_py = module_dir / f"{stem}_int.py"
                elif definite_wires and definite_location:
                    out_py = module_dir / f"{stem}_definite_wires.py"
                else:
                    out_py = module_dir / f"{stem}_definite_pins.py"
                out_sch = module_dir / f"{stem}_out.kicad_sch"

                # if out_py.exists():
                #     skipped_files += 1
                #     print(f"⏩ Skip [{module_name}/{sch_path.name}] because output script exists: {out_py.name}")
                #     continue
