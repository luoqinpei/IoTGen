'''
This module provides functions for comparing netlists and identifying differences between them.
It includes the following criterions:
1. Jaccard similarity of net sets (where each net is represented as a set of "lib:part:pin" tokens).
2. Precision, recall, and F1-score of net sets, treating one netlist as "ground truth" and the other as "predicted".
The main function demonstrates how to use these utilities to compare two netlist files and print the results.

Note that we also implement a heuristic to drop "dangling" single-node nets that are likely unconnected and whose net name describes that lone node itself (e.g. "unconnected-(C1-Pad1)"). This helps focus the comparison on meaningful nets that connect multiple components.
'''
