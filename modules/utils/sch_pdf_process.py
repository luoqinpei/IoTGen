...
This module provides utilities for processing PDF files, specifically for cropping a PDF page and overlaying axes using OpenCV. It includes functions to render a PDF page to a numpy array, trim borders from an image, get the size of an image, perform center cropping, and overlay axes with matplotlib. The main function demonstrates how to use these utilities to process a schematic PDF and save the resulting images with axes overlaid.

Cropping the schematic PDF is sometimes useful for LLM to focus on the relevant content and ignore extraneous whitespace. Overlaying axes can help the LLM understand the spatial layout of the schematic, which may improve its ability to reason about component placement and connections.
"""
