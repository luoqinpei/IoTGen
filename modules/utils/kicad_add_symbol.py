'''
This script provides utility functions to add symbols from a KiCad symbol library to a KiCad schematic file. It includes functions to parse and format S-expressions, read and write KiCad schematic and symbol files, find symbols in the library, create symbol instances, and check for overlaps with existing symbols in the schematic. The main function demonstrates how to use these utilities to add a symbol from a library to a schematic at a specified location with given properties.
'''
